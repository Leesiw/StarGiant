...
ID3D12RootSignature* CScene::CreateGraphicsRootSignature(ID3D12Device* pd3dDevice)
{
	...
	pd3dDescriptorRanges[8].BaseShaderRegister = 1; //t1: gtxtGodRaySceneTexture
	pd3dDescriptorRanges[8].RegisterSpace = 0;
	pd3dDescriptorRanges[8].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[9].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[9].NumDescriptors = 1;
	pd3dDescriptorRanges[9].BaseShaderRegister = 2; //t2: gtxtGodRayDepthTexture
	...
	pd3dDescriptorRanges[14].BaseShaderRegister = 18; //Depth Buffer
	...
	pd3dRootParameters[13].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[8]); //Scene buffer
	pd3dRootParameters[13].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[14].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[14].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[14].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[9]);//SceneDepth buffer 
	...
	pd3dRootParameters[20].DescriptorTable.pDescriptorRanges = &pd3dDescriptorRanges[14]; //Depth Buffer
	...
	pd3dRootParameters[25].Descriptor.ShaderRegister = 6; //ToLight
...
void CScene::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	//if(aafsdaa ==0)
	//setParticleStarts(MAX_CIRCLE_PARTICLES, { 0,0,0 }, 0);
	//for (int i = 0; i < MAX_CIRCLE_PARTICLES; ++i) {
	//	if (!b_Inside) {
	//		m_pFlameParticle[i]->setTarpos(m_pPlayer[0]->GetPosition());
	//		m_pFlameParticle[i]->SetLookAt(xmf3CameraPosition, XMFLOAT3(0.0f, 1.0f, 0.0f));
	...
	//	}
	//}
	////{ 0,0,0 }  m_pPlayer[0]->GetPosition()
	//if (aafsdaa == 0)

	//setParticleStarts(MAX_CIRCLE_PARTICLES, { 0,0,0 }, 1);
	//for (int i = 0; i < MAX_CIRCLE_PARTICLES; ++i) {
	//	if (!b_Inside) {
	//		m_pSkull[i]->setTarpos(m_pPlayer[0]->GetPosition());
	//		m_pSkull[i]->SetLookAt(xmf3CameraPosition, XMFLOAT3(0.0f, 1.0f, 0.0f));
...
void CScene::setParticleStarts(int cnt, XMFLOAT3 tarPos, int num)
{
	if(num==0)
		for (int i = 0; i < cnt; i++)
		{
			m_pFlameParticle[i]->isLive = true;

...
void CGameFramework::FrameAdvance()
{
	...
	if (b_Inside) {
		...
		m_pInsideScene->OnPreRender(m_pd3dCommandList, m_pCamera);
	}
	else
	{
		m_pScene->OnPrepareRender(m_pd3dCommandList);
		m_pScene->OnPreRender(m_pd3dCommandList, m_pInsideCamera);
	}
...
wstring CGameFramework::ChangeMission(MissionType mType)
{
	...
	realCenterDist = to_wstring(distanceCenter_real);
...
void CGameFramework::ProcessPacket(char* p)
{
	...
	case SC_MISSION_START:
	{
		...
		if (curMissionType == MissionType::CS_BAD_ENDING) {
			...
			//if (m_pInsidePlayer && b_Inside)for (int i = 0; i < 3; ++i) {
			//	if (m_pInsidePlayer[i]->isAlive) {
			//		m_pInsidePlayer[i]->curMissionType= MissionType::CS_BAD_ENDING;
			//	}
			//}
		}

...
CAttactCamera::CAttactCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		if (pCamera->GetMode() == ATTACT_CAMERA_C|| pCamera->GetMode() == DRIVE_CAMERA)
		{
			cout << "\nC";
		}

		else if (pCamera->GetMode() == ATTACT_CAMERA_L)
		{
			cout << "\nL ";
			m_xmf3Look.x += 90.0f;
		...
		else if (pCamera->GetMode() == ATTACT_CAMERA_R)
		{
			cout << "\nR";
			m_xmf3Look.x -= 90.0f;
...
CAttact_rCamera::CAttact_rCamera(CCamera* pCamera) : CAttactCamera(pCamera)
{
	m_nMode = ATTACT_CAMERA_R;
...
CAttact_lCamera::CAttact_lCamera(CCamera* pCamera) : CAttactCamera(pCamera)
{
	m_nMode = ATTACT_CAMERA_L;
